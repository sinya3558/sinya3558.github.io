...
# 22. OpenCV 를 이용한 기하학적 변환 Part.1
...
- 영상 필터링에서는 이미지의 *가장자리(엣지)* 가 크게 중요하지 않기 때문에, 적당한 값으로 채우거나(제로 패딩), 일부 영역을 제거하는 방식으로 처리한다

<br>

## 서로 다른 필터에 따른 영상(이미지) 변환결과 비교

- <u>각 필터는 고유한 Kernel 값을 이미 가지고 있다.</u> 이제 필터별로 입력 이미지가 어떻게 변환되는지 결과를 비교해보자
...
1. 평균값 필터 :
    ...
    - 영상의 특정 좌표 값을 주변 픽셀 값들의 산술 평균으로 설정
        ...
        - 필터의 크기가 작을 수록(3x3) 블러링이 잘될까? 클수록(5x5) 잘될까?
            - 5x5 의 필터가 블러링이 더 잘 된다. 왜냐면 **더 넓은 영역의 필터 사이즈**를 가지고 평균값을 내기때문에 주변 픽셀들의 영향을 더 많이 받아 결과 이미지가 **더 흐릿해지는** 특징이 있다.
...
2. 양뱡향 
    ...
    - 가우시안 필터의 장점을 보완한 필터
        - 픽셀 안의 **명암비(색상 차이)가 극명한 영역**에서는, 컨볼루션을 진행 하지 않도록 **가중치를 낮춰** 불필요한 블러링을 줄이는 기법
        
    - 양방항 필터에서 중요하게 다루는 두 가지 요소
        1. `공간적 거리 (Spatial Closeness)` : 필터의 중심 픽셀로부터 물리적인 거리가 가까운 픽셀에 더 큰 가중치를 부여한다(가우시안과 동일한 원리)
        2. `강도 유사성 (Intensity Similarity)` : 필터의 중심 픽셀ㄹ과 강도(밝기 또는 색상) 값이 비슷한 픽셀일수록 더 큰 가중치를 부여한다
