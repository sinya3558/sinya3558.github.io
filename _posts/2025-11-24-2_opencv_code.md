---
title:  "OpenCV (1) : 기초 이미지 처리 & 필터링 실습"
...
### 1-1. 평균값 필터 블러링 (mean blur filter) 
...
2. blur 하는 법 :
    ...
    # src : 입력영상
    # ksize, mean filter 의 kernel size
    ```
...
### 1-2. `blur 함수`를 사용하여 위와 동일한 블러링 적용하기(convolution)
...
### 2-2. `GaussianBlur 함수` 사용해서 가우시안 필터 적용하기
...
# src : 입력영상
# k_size : mean filter kernel size,  (0, 0) 자동으로 결정 -> 이건 거의 따로 지정하지 X, 대신 시그마를 지정해줌
# sigmaX : gaussian x 방향의 표준편차, sigma = 1
# sigmaY : gaussian Y 방향의 표준편차(시그마)
...
> ***결론*** : 시그마가 커질수록 더 넓은 영역을 평균하므로 이미지가 더 부드럽고 흐릿해진다.
...
### 3-1. 샤프닝 (Sharpening) 필터

> 블러 처리된 이미지에서 원본을 강조하는 방식으로, 엣지(경계선)를 더 선명하게 만든다

- Sharpening 필터 사용을 위해서는 <u>먼저 Gaussian Blur가 필수적으로 사용된다</u>
...
#### 결과 비교 : Gaussian blur & sharpening 
...
###  4-1. 양방향 필터 (Bilateral Filter)

> 가우시안 필터의 단점을 보완한 블러링 기법으로, 처리 시간은 더 오래 걸리지만 보다 원본 정보를 보존한 상태의 블러링을 제공
...
## 표준편차 설명 : 주변 범위를 얼마나 포함할것인가 (가우시안 커브의 너비 (크면 납작 호떡, 작으면 유니콘 뿔))
# sigmaColor : 색공간의 표준편차 (색상에 대하여)
# sigmaSpace : 물리적 좌표공간의 표준편차 (공간에 대해서)
...
#### : Gaussian vs Bilateral 필터의 블러링 효과 비교

- original / gaussian / bilateral filter 

![image.png](/assets/img/opencv/2_opencv2_실습_files/스크린샷%202025-11-24%20224257.png)

- 가우시안과 양방향 둘 다 블러처리 되었으나, 양방향 필터가 상당히 엣지를 더 선명하게 유지하면서 원본의 특징값을 보존한채로 블러링 된 상태
...
2. 카툰 필터링의 실행 순서
    ...
    - (1) 블러링으로 색 영역을 부드럽게 변환

    - (2) 엣지 검출로 윤곽선을 강조

    - (3) 두 결과를 bitwise 연산으로 결합하여 만화풍 표현을 생성
